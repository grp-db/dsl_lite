...
def build_preset_header(*, model_endpoint: str, source_name: str, source_type: str,
                        raw_sample_path: str, source_table: str,
                        preset_parsed: dict, refined: bool = False) -> str:
    """
    Build the top-of-file comment block from runtime params and the parsed preset YAML.
    Extracts developer email, table names, autoloader path, and gold filters automatically.
    """
    # --- developer email from Databricks cluster tags ---
    developer = ""
    try:
        developer = spark.conf.get("spark.databricks.clusterUsageTags.userEmail", "")
    except Exception:
        pass

    # --- extract structure from the parsed YAML ---
    autoloader_path = ""
    try:
        inputs = preset_parsed.get("autoloader", {}).get("inputs") or []
        autoloader_path = inputs[0] if inputs else ""
    except Exception:
        pass

    bronze_name = ""
    try:
        bronze_name = preset_parsed.get("bronze", {}).get("name", "")
    except Exception:
        pass
    ...
    try:
        silver_names = [
            t.get("name", "")
            for t in (preset_parsed.get("silver", {}).get("transform") or [])
        ]
    except Exception:
        pass

    gold_entries = []  # [(name, filter_str), ...]
    try:
        for g in (preset_parsed.get("gold") or []):
            gold_entries.append((g.get("name", ""), g.get("filter") or ""))
    except Exception:
        pass

    gold_names = [n for n, _ in gold_entries]
    ...
    if gold_entries:
        covers = [
            f"{_title(name)}: {filt}" if filt else _title(name)
            for name, filt in gold_entries
        ]
        if len(", ".join(covers)) <= 100:
    ...
    lines.append(f"# Source:     {source_name} / {source_type}")
    raw_path = autoloader_path or raw_sample_path or source_table or "(unspecified)"
    lines.append(f"# Raw path:   {raw_path}")
    if bronze_name:
        lines.append(f"# Bronze:     {bronze_name}")
    if silver_names:
        lines.append(f"# Silver:     {', '.join(silver_names)}")
    if gold_names:
        lines.append(f"# Gold:       {', '.join(gold_names)}")
    date_str    = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    refined_tag = " (refined)" if refined else ""
    lines.append(f"# Generated:  {date_str}{refined_tag}  |  Model: {model_endpoint}")

    return "\n".join(lines) + "\n\n"


def splice_gold(base_text: str, generated_yaml_text: str) -> str:
    """
...
def assemble_final(*, preset_yaml: str, existing_preset_text, target_layers: str) -> str:
    """
    For target_layers='gold' with an existing preset, splice the new gold block into
    the existing bronze/silver bytes. Otherwise return the model output as-is.
    The caller is responsible for prepending any file header.
    """
    if target_layers == "gold" and existing_preset_text is not None:
        return splice_gold(existing_preset_text, preset_yaml)
    return preset_yaml
