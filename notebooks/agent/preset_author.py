...
# Provenance header — injected above generated content so future reviewers can see
# what produced the preset (or the spliced gold section) without digging through git.
# All-comment YAML is valid and ignored by safe_load.
_provenance_source = source_table or raw_sample_path or "(unspecified)"
_provenance = "\n".join([
    "# ── Generated by preset_author agent ─────────────────────────────────────",
    f"# Generated:  {datetime.now(timezone.utc).isoformat(timespec='seconds')}",
    f"# Model:      {model_endpoint}",
    f"# Skill:      {skill_path}",
    f"# Source:     {_provenance_source}",
    f"# Input:      input_layer={input_layer}, ocsf_classes={ocsf_classes or '(inferred)'}",
    "# ─────────────────────────────────────────────────────────────────────────",
]) + "\n"
...
if input_layer == "silver" and existing_preset_text is not None:
    ...
    final_yaml = _provenance + preset_yaml
    print("── FINAL PRESET ────────────────────────────────────────")
...
if feedback.strip() and not feedback.strip().startswith("#"):
    ...
    preset_yaml = refined
    # Refresh the timestamp so the provenance header reflects the refine turn.
    _refined_provenance = _provenance.replace(
        _provenance.splitlines()[1],
        f"# Generated:  {datetime.now(timezone.utc).isoformat(timespec='seconds')} (refined)",
    )
    if input_layer == "silver" and existing_preset_text is not None:
        final_yaml = _splice_gold(existing_preset_text, preset_yaml, provenance=_refined_provenance)
    else:
        final_yaml = _refined_provenance + preset_yaml
    print(final_yaml)
else:
