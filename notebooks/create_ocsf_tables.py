...
spark.sql(f"""CREATE OR REPLACE TABLE `{catalog_name}`.`{gold_database}`.kernel_extension_activity (
  ...
  type_uid BIGINT,
  category_name STRING,
  metadata STRUCT<uid: STRING, profiles: ARRAY<STRING>, product: STRUCT<vendor_name: STRING, name: STRING, version: STRING>, tenant_uid: STRING, event_code: STRING>,
  severity STRING,
  severity_id INT,
  class_uid INT,
